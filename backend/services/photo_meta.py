...
def _meta_to_row(rel: str, meta: dict, mtime: float | None) -> tuple:
    return (
        rel,
...
async def _get_mtime_limited(abs_path: str) -> float | None:
    async with _exif_read_semaphore:
        return await asyncio.to_thread(_safe_getmtime, abs_path)


async def load_photo_meta(rels: list[str], db) -> dict[str, dict]:
    """rel 경로 목록의 EXIF 메타를 photo_meta_cache에서 일괄 조회, 미스는 파일에서 읽어 캐시.

    캐시 행에 저장된 mtime과 파일의 현재 mtime이 다르면(외부 앱 등으로 파일 내용이
    바뀐 경우) 캐시를 무효로 보고 다시 읽는다 — mtime 비교 자체도 세마포어
    (EXIF_READ_CONCURRENCY)로 동시 실행을 제한해 대량 조회 시 NAS에 stat 호출이
    한꺼번에 몰리지 않도록 한다.

    미스 읽기도 같은 세마포어로 동시 실행 개수를 제한 — 캐시 미스가 대량(예: search
    날짜 필터 첫 조회)이어도 NAS I/O가 한꺼번에 몰리지 않도록 한다."""
    root = _photo_root()

    def _abs(p: str) -> str:
        return p if os.path.isabs(p) else os.path.join(root, p)

    cached_rows: dict[str, object] = {}
    for i in range(0, len(rels), _CACHE_CHUNK):
        ...
        async with db.execute(
            ...
            for row in await cur.fetchall():
                cached_rows[row["file_path"]] = row

    meta_by_rel: dict[str, dict] = {}
    if cached_rows:
        current_mtimes = await asyncio.gather(*[
            _get_mtime_limited(_abs(rel)) for rel in cached_rows
        ])
        for rel, current_mtime in zip(cached_rows, current_mtimes):
            row = cached_rows[rel]
            if current_mtime is not None and row["mtime"] is not None \
                    and abs(current_mtime - row["mtime"]) > 1e-6:
                continue  # 파일이 캐시 이후 변경됨 → 미스로 취급, 다시 읽음
            meta_by_rel[rel] = _row_to_meta(row)
    ...
    if uncached:
        ...
        mtimes = await asyncio.gather(*[
            _get_mtime_limited(_abs(r)) for r in uncached
        ])
        for rel, meta in zip(uncached, metas):
            meta_by_rel[rel] = meta
        # 읽기 성공(width not None)한 경우만 캐시 저장 — 실패 결과는 저장 안 해 다음 요청에 재시도
        inserts = [_meta_to_row(rel, meta, mtime) for rel, meta, mtime in zip(uncached, metas, mtimes)
