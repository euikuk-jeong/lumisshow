...
async def test_path_tag_reset_job_create_and_dedup(admin_client):
    """폴더 태그 재계산(path_tag_reset)도 동일한 일반 잡 생성·중복 방지 경로를 탄다."""
    r = await admin_client.post("/api/admin/ai/jobs", json={"type": "path_tag_reset"})
    assert r.status_code == 201 and r.json()["duplicated"] is False
    job_id = r.json()["id"]

    r = await admin_client.post("/api/admin/ai/jobs", json={"type": "path_tag_reset"})
    assert r.json() == {"id": job_id, "type": "path_tag_reset", "duplicated": True}

    stats = (await admin_client.get("/api/admin/ai/status")).json()
    assert stats["recent_jobs"][0]["type"] == "path_tag_reset"
    assert stats["recent_jobs"][0]["status"] == "pending"
