...
async def test_failed_exif_not_cached(auth_client, monkeypatch):
    ...
    from backend.services.thumbnail import _EMPTY_META, get_media_meta as real_get_meta

    # 1차 browse: EXIF 읽기 실패 시뮬레이션
    monkeypatch.setattr(photo_meta, "get_media_meta", lambda fp: dict(_EMPTY_META))
    r1 = await auth_client.get("/api/admin/browse")
    assert r1.status_code == 200
    assert all(p["width"] is None for p in r1.json()["photos"]), "실패 시 width=None이어야 함"

    # 원래 함수 복구
    monkeypatch.setattr(photo_meta, "get_media_meta", real_get_meta)
...
async def test_load_photo_meta_respects_exif_concurrency_limit(client, monkeypatch):
    ...
    monkeypatch.setattr(photo_meta, "get_media_meta", slow_get_image_meta)
